/*
 Vue.js Geocledian Dashboard
 
 init script
 
 created: 2022-09-09, jsommer
...
const libs = ['https://unpkg.com/vue@2.6.14/dist/vue.min.js',
              ...
              '../gc-netarea/js/gc-netarea.min.js',
              '../gc-similarity-control/js/gc-similarity-control.min.js',
              '../gc-performance/js/gc-performance.min.js',
            ];
...
function gcGetBaseURL() {
    ...
    let scriptURL = document.getElementById("gc-dashboard-performance-init");
...
function initComponent() {
    ...
    */
    ...
    vmRoot = new Vue({
      ...
      i18n: i18n,
      created() {
        console.debug("gc-dashboard-performance-init created!");
      ...
      mounted: function () {
        ...
        this.$on('queryDateStatsChange', this.queryDateStatsChange);
        ...
        this.$on('getSimilarity', this.getSimilarity);
        ...
        this.$on('simVerificationChange', this.simVerificationChange);

        this.containerSplitSize = [50,50];
      ...
      methods: {
        ...
        queryDateStatsChange: function (queryDateStats) {
          // set only if changed, otherwise setter fires every time in children's queryDateStats properties!
        ...
        simStartdateChange: function (value) {
          this.simStartdate = value;
        },
        ...
        simRadiusChange: function (value) {
          this.simRadius = value;
        },
        simReferencesChange: function (value) {
          this.simReferences = value;
        },
        simVerificationChange: function (value) {
          this.simVerification = value;
        },
        ...
        getSimilarity: function () {
          for (let i=0; i < this.$children.length; i++ ) {
            if (this.$children[i].gcWidgetId.includes("chart")) {
              this.$children[i].getSimilarity();
            }
          }
        },
        resetSimilarity: function () {
          for (let i=0; i < this.$children.length; i++ ) {
            if (this.$children[i].gcWidgetId.includes("chart")) {
              this.$children[i].similarity = {
                "reference_timeseries" : [], 
                "parcel_timeseries" : [],
                "similarity": {},
                "classification": {} 
              };
              this.$children[i].currentGraphContent = "statistics";
              // clear chart
              this.$children[i].chart.unload();
            }
          }
        },
        ...
        setLocaleForIndexPage() {
          ...
          // document.getElementById("tabGraph2").innerHTML = i18n.t("indexLocales.headings.graph2");
